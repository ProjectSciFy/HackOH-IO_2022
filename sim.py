...
def setup1():
    ...
    for i in range(num):
        ...
        tempCar = CAR.Car((location[0], location[1] - i * 180), speed, direction1, lane, size, i, lanes, lane_left_to_right)
        if direction1 == "W":
    ...
    create_roads('horizontal', int(lanes), location[0], location[1] + size[1] / 2, 40)  
    updateCars(cars, lanes)


def setup2():
    with open("1c_2l_W.txt") as f:
        input = f.readlines()
    num = int(input[0])
    lanes = int(input[1])
    direction1 = str(input[2])
    ...
    for i in range(num):
        lane = random.randrange(int(lanes))
        tempCar = CAR.Car((location[0], location[1]), int(speed/2), direction1, lane, size, i, lanes, lane_left_to_right)
        if direction1 == "W":
            tempCar.setRoute((0, 300 - lane_left_to_right[0] * lane))
        if direction1 == "N":
            tempCar.setRoute((400 + lane_left_to_right[0] * lane, 0))
...
def setup3():
    ...
    for i in range(num):
        lane = random.randrange(int(lanes))
        tempCar = CAR.Car((location[0] + (i * 80), location[1] - lane_left_to_right[0] * lane - lane * 10), random.randint(2, int(speed/2)), direction1, lane, size, i, lanes, lane_left_to_right)
        if direction1 == "W":
            tempCar.setRoute((0, 300 - lane_left_to_right[0] * lane))
        if direction1 == "N":
            tempCar.setRoute((400 + lane_left_to_right[0] * lane, 0))
        cars.append(tempCar)
        tempCar.addCarToAllCars(tempCar)
        print(f"\tStart of car {i} heading {direction1} in lane {lane}: ", cars[i].getState())
    create_roads('horizontal', int(lanes), location[0], location[1] - 3 * size[1] / 2, 40)  
    updateCars(cars, lanes)
    
def setup4():
    with open("10c_3l_W.txt") as f:
        input = f.readlines()
    num = int(input[0])
    lanes = int(input[1])
    direction1 = str(input[2])
    ...
    for i in range(num):
        lane = random.randrange(int(lanes))
        tempCar = CAR.Car((location[0] + (i * 110), location[1] - lane_left_to_right[0] * lane - lane * 10), random.randint(2, int(speed/2)), direction1, lane, size, i, lanes, lane_left_to_right)
        if direction1 == "W":
            tempCar.setRoute((0, 300 - lane_left_to_right[0] * lane - lane * 10))
        if direction1 == "N":
            tempCar.setRoute((400 + lane_left_to_right[0] * lane, 0))
        cars.append(tempCar)
        tempCar.addCarToAllCars(tempCar)
